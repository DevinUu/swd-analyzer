...
for file in cmake_file:
	...
	#新增起始
	contents = contents.replace( original_name + "Types", new_analyzer_name + "Types" )
	contents = contents.replace( original_name + "Utils", new_analyzer_name + "Utils" )
	contents = contents.replace( original_name.lower() + "_analyzer", new_analyzer_name.lower() + "_analyzer" )
	#新增结束
...
for file in new_files:
	...
	contents = contents.replace( original_name + "Types", new_analyzer_name + "Types" )
	contents = contents.replace( original_name + "Utils", new_analyzer_name + "Utils" )
	contents = contents.replace( original_name.upper() + "_TYPES_", new_analyzer_name.upper() + "_TYPES_" )
	#contents = contents.replace( "Simple Serial", new_analyzer_title )
	contents = contents.replace( "\"SWD\"", "\""+new_analyzer_title+"\"" )
	#新增结束
	open( file, 'w' ).write( contents )